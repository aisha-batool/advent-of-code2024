...
def bfs(matrix, graph, queue, path, visited, requiredN):
    if queue == []:
        return
    val = queue.pop(0)
    ...
    for i in graph[val]:
        if matrix[i[0]][i[1]]==requiredN:
            queue.append(i)
            bfs(matrix, graph, queue, path, visited, requiredN+1)
            # queue.pop()
    # return visited
import pprint
with open('inputted.txt') as f:
    ...
    for i in range(len(matrix)):
        for j in range(len(matrix)):
            if matrix[i][j] == 0:
                bfs(matrix, graph, [(i, j)], path, [(i, j)], 1)
    lastSame = {}
